...
export default function AdminLogin({ onLogin, onSwitchToRegister }) {
  ...
  return (
    <>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-pink-500 via-rose-500 to-fuchsia-600">
          <div className="text-center relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div
                className="w-48 h-48 bg-white/20 rounded-full animate-ping"
                style={{ animationDelay: "0.5s", animationDuration: "2s" }}
              ></div>
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div
                className="w-64 h-64 bg-white/10 rounded-full animate-ping"
            ...
            <div className="relative z-10 mb-8">
              <svg
                className="w-32 h-32 text-white mx-auto drop-shadow-2xl animate-bounce"
                fill="currentColor"
                viewBox="0 0 24 24"
            ...
            <div className="relative z-10 flex justify-center gap-4 mt-10">
              ...
              <div
                className="w-5 h-5 bg-white rounded-full animate-bounce shadow-lg"
            ...
            <div
              className="absolute top-20 right-16 w-2 h-2 bg-white rounded-full animate-ping"
              style={{ animationDelay: "0.3s" }}
            ></div>
            <div
              className="absolute bottom-16 left-20 w-4 h-4 bg-white rounded-full animate-ping"
              style={{ animationDelay: "0.6s" }}
            ></div>
            <div
              className="absolute bottom-10 right-10 w-3 h-3 bg-white rounded-full animate-ping"
      ...
      {/* Main Login Screen */}
      <div
        className={`fixed inset-0 bg-gradient-to-br from-pink-50 via-rose-50 to-pink-100 overflow-auto transition-all duration-1000 ${
          isLoading ? "scale-150 opacity-0 blur-2xl rotate-12" : "scale-100 opacity-100 blur-0 rotate-0"
        }`}
      >
        <div className="min-h-screen flex items-center justify-center p-6">
          <div
            className={`w-full max-w-5xl flex items-center gap-12 transition-all duration-1000 ${
              isLoading ? "-translate-y-32 opacity-0 scale-75" : "translate-y-0 opacity-100 scale-100"
            }`}
          >
            {/* Image Section */}
            <div
              className={`hidden lg:flex flex-1 items-center justify-center transition-all duration-1000 ${
                isLoading ? "-translate-x-20 opacity-0" : "translate-x-0 opacity-100"
              }`}
            >
              <img
            ...
            >
              ...
              <div className="bg-white/80 backdrop-blur-sm p-10 rounded-2xl shadow-xl border border-pink-100 hover:shadow-2xl transition-shadow">
                <h2 className="text-2xl font-serif text-gray-800 mb-8 text-center">Admin Login</h2>
