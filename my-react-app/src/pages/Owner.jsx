            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-pink-100 to-rose-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Pets</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOwners.length > 0 ? (
                  filteredOwners.map((o) => (
                    <tr key={o.ownerId} className="hover:bg-pink-50 transition-colors duration-150">
                      <td className="px-6 py-4">{o.firstName} {o.lastName}</td>
                      <td className="px-6 py-4">{o.email}</td>
                      <td className="px-6 py-4">{o.phoneNumber}</td>
                      <td className="px-6 py-4">{(o.pets || []).length}</td>
                      <td className="px-6 py-4 flex gap-2">
                        <button onClick={() => setModal({ type: "view", owner: o })} className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200">View</button>
                        <button onClick={() => setModal({ type: "edit", owner: o })} className="px-3 py-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200">Edit</button>
                        <button onClick={() => deleteOwner(o.ownerId)} className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200">Delete</button>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="5" className="px-6 py-12 text-center text-gray-500">No owners found</td>
                  </tr>
                )}
              </tbody>
            </table>
...
function OwnerModal({ type, owner, addOwner, editOwner, close }) {
  const [formData, setFormData] = useState(owner || { firstName: "", lastName: "", email: "", phoneNumber: "", address: "" });
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="bg-gradient-to-r from-pink-500 to-rose-500 px-6 py-5 rounded-t-2xl flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">{type === "view" ? "Owner Details" : type === "edit" ? "Edit Owner" : "Add New Owner"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {type !== "view" && <button type="submit" className="bg-gradient-to-r from-pink-500 to-rose-500 text-white px-6 py-3 rounded-xl font-semibold">Save</button>}
          <button type="button" onClick={close} className="ml-2 px-6 py-3 rounded-xl border">Cancel</button>
        </form>
      </div>
    </div>
  );
